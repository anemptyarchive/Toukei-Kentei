...
dir_path += 'estimate_mean_known_variance/' # フォルダを指定
print(dir_path)
...
from matplotlib.offsetbox import AnchoredText # ラベルの表示用 
...
def update(i):
    ...
    pop_param_lbl += '$L = \\bar{x}_{obs} - z_{\\frac{\\alpha}{2}} \\sigma_{smp} = '+f'{ci_bound_lower:.2f}$\n'
    pop_param_lbl += '$U = \\bar{x}_{obs} + z_{\\frac{\\alpha}{2}} \\sigma_{smp} = '+f'{ci_bound_upper:.2f}$'
    ...
    smp_param_lbl += '\\sigma_{smp} = \\frac{\\sigma_{pop}}{\\sqrt{N}} = '+f'{sigma_smp:.2f}$'
    ...
    ax2x.set_xticks(
        ...
        labels=[
            '$\\bar{x}_{obs}$', 
            '$\\bar{x}_{obs} - z_{\\frac{\\alpha}{2}} \\sigma_{smp}$', 
            '$\\bar{x}_{obs} + z_{\\frac{\\alpha}{2}} \\sigma_{smp}$'
        ]
    ) # 信頼区間のラベル
    ...
    # 目盛ラベルを調整(ラベルが重なる対策用)
    halignments = ['center', 'left', 'left'] # 表示位置を指定
    rotations   = [0, 15, 15] # 表示角度を指定
    ...
    # z軸の範囲を設定
    z_min = (x_min - mu_smp) / sigma_smp
    z_max = (x_max - mu_smp) / sigma_smp
    ...
    ax2x.set_xticks(
        ticks =[
            z_obs, 
            cr_bound_lower, cr_bound_upper, 
            z_obs+cr_bound_lower, z_obs+cr_bound_upper
        ], 
        labels=[
            '$z_{obs}$', 
            '$z_{1-\\frac{\\alpha}{2}}$', '$z_{\\frac{\\alpha}{2}}$', 
            '$z_{obs}-z_{\\frac{\\alpha}{2}}$', '$z_{obs}+z_{\\frac{\\alpha}{2}}$'
        ]
    ) # 中央領域のラベル
    ...
    # 目盛ラベルを調整(ラベルが重なる対策用)
    halignments = ['center', 'center', 'center', 'left', 'left'] # 表示位置を指定
    rotations   = [0, 0, 0, 15, 15] # 表示位置を指定
    labels      = ax2x.get_xticklabels() # 軸情報を取得
    for label, ha, r in zip(labels, halignments, rotations):
        label.set_ha(ha)
        label.set_rotation(r)
...
# 動画を書出
anim.save(
    filename=dir_path+'mean_ci_n.gif', 
...
def update(I):
    ...
    pop_param_lbl  = '$\\mu_{pop} = '+f'{mu_pop:.2f}, '
    pop_param_lbl += '\\sigma_{pop} = '+f'{sigma_pop:.2f}$\n'
    pop_param_lbl += '$L_i = \\bar{x}_i - z_{\\frac{\\alpha}{2}} \\sigma_{smp} = '+f'{ci_bound_lower:.2f}$\n'
    pop_param_lbl += '$U_i = \\bar{x}_i + z_{\\frac{\\alpha}{2}} \\sigma_{smp} = '+f'{ci_bound_upper:.2f}$'
    ...
    ax2x.set_xticks(
        ...
        labels=[
            '$\\bar{x}_i$', 
            '$\\bar{x}_i - z_{\\frac{\\alpha}{2}} \\sigma_{smp}$', 
            '$\\bar{x}_i + z_{\\frac{\\alpha}{2}} \\sigma_{smp}$'
        ]
    ) # 信頼区間のラベル
    ax.set_ylabel('$N(\\bar{x} \\mid \\mu_{smp}, \\sigma_{smp}^2)$')
    ...
    ax2x.set_xticks(
        ...
        labels=[
            '$z_i$', 
            '$z_i-z_{\\frac{\\alpha}{2}}$', '$z_i+z_{\\frac{\\alpha}{2}}$'
        ]
    ) # 中央領域のラベル
    ax.set_ylabel('$N(z \\mid 0, 1)$')
    ax.grid()
...
# 動画を書出
anim.save(
    filename=dir_path+'mean_ci_iter.gif', 
    progress_callback=lambda i, n: print(f'\rframe: {i+1} / {n}', end='', flush=True)
)
