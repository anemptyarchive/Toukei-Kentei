...
dir_path += 'estimate_mean_difference_unknown_variance/' # フォルダを指定
print(dir_path)
...
# 標準化分布のp軸の範囲を設定
u = 0.5
Pt_max = t.pdf(x=0.0, df=np.sum([N_1_base+N, N_2_base+N])-2, loc=0.0, scale=1.0) # 最終試行の最頻値
...
fig.suptitle('Population Mean Difference Confidence Interval (unknown variance)', fontsize=20)
...
def update(i):
    ...
    # 中央領域の範囲を計算
    cr_bound_upper = t.ppf(q=1.0-0.5*alpha, df=np.sum(N_lt)-2, loc=0.0, scale=1.0)
    ...
    # t軸の範囲を設定
    if np.sum(N_lt) > 2:
        t_min = (d_min - d_obs) / sigma_smp
        t_max = (d_max - d_obs) / sigma_smp
    else: # 自由度の条件を満たさない場合 
        # 標本統計量を計算
        tmp_N_lt       = [N_lt[0]+1, N_lt[1]+1]
        tmp_x_bar_lt   = [np.mean(x_n[:n]) for x_n, n in zip(x_lt, tmp_N_lt)]
        tmp_sigma2_hat = [np.var(x_n[:n], ddof=1) for x_n, n in zip(x_lt, tmp_N_lt)]
        tmp_d_obs      = np.subtract(*tmp_x_bar_lt)
        tmp_sigma_smp  = np.sqrt(np.sum([sgm2 / n for sgm2, n in zip(tmp_sigma2_hat, tmp_N_lt)]))
        t_min = (d_min - tmp_d_obs) / tmp_sigma_smp
        t_max = (d_max - tmp_d_obs) / tmp_sigma_smp

    # t軸の値を作成
    t_vec = np.linspace(start=t_min, stop=t_max, num=1001)

    # 標準化分布の確率密度を計算
    std_dens_vec = t.pdf(x=t_vec, df=np.sum(N_lt)-2, loc=0.0, scale=1.0)

    # 中央領域を計算
    cr_t_vec    = np.linspace(start=cr_bound_lower, stop=cr_bound_upper, num=501)
    cr_dens_vec = t.pdf(x=cr_t_vec, df=np.sum(N_lt)-2, loc=0.0, scale=1.0)
    ...
    ax.set_ylabel('$N(t \\mid n_1+n_2-2, 0, 1)$')
...
# 標準化分布のp軸の範囲を設定
u = 0.5
Pt_max = t.pdf(x=0.0, df=np.sum(N_lt)-2, loc=0.0, scale=1.0) # 最頻値
Pt_max = np.ceil(Pt_max /u)*u # u単位で切り上げ
print('p(t) size:', Pt_max)
...
def update(I):
    ...
    tail_dens_vec = t.pdf(x=tail_t_vec, df=np.sum(N_lt)-2, loc=0.0, scale=1.0)
    ...
    ax.set_ylabel('$t(t \\mid n_1+n_2-2, 0, 1)$')
    ax.set_title(std_param_lbl, loc='left')
    #ax.legend(loc='upper right', prop={'size': 8})
    ax.grid()
    ax.set_xlim(xmin=t_min, xmax=t_max)   # (目盛の共通化用)
